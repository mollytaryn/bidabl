feature "User signs in" do

  before do
    page.driver.block_unknown_urls
    visit root_path
    find("#sign-in-form").click
  end

  scenario "Returning customer signs in", :js => true do
    user = Fabricate(:user, username: "Jennyb")
    fill_in "Email", with: user.email
    fill_in "Password", with: "password1"
    find("#sign-in").click
    current_path.should == user_path(user)
    page.should have_content("Welcome back, Jennyb")
    page.should_not have_content("Sign In")
    page.should_not have_content("Sign Up")
    page.should have_content("Sign Out")
    # Smoke testing Sign Out:
    click_on "Sign Out"
    page.should have_content("You have been signed out")
    page.should have_content("Sign In")
    page.should_not have_content("Sign Out")
  end

  scenario "Returning user attempts signin with incorrect password", :js => true do
    user = Fabricate(:user, username: "Bob")
    fill_in "Email", with: user.email
    fill_in "Password", with: "wrongpassword"
    find("#sign-in").click
    page.should have_content("We could not sign you in. Please check your email/password and try again.")
    field_labeled("Email").value.should == user.email
    fill_in "Password", with: "password1"
    click_button "Enter"
    page.should have_content("Welcome back, Bob")
  end

  scenario "User signs in with wrong email", :js => true do
    Fabricate(:user, email: "susie@example.com", password: "ThisIsAwesome", password_confirmation: "ThisIsAwesome")
    fill_in "Email", with: "joe@example.com"
    fill_in("Password", with: "ThisIsAwesome")
    click_on "Enter"
    page.should have_content("We could not sign you in. Please check your email/password and try again.")
  end

  scenario "User signs in with blanks", :js => true do
    click_on "Enter"
    page.should have_content("We could not sign you in. Please check your email/password and try again.")
  end
end