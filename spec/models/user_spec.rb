...
RSpec.describe User, type: :model do
  describe "validations" do
    ...
    it "should allow valid values for email" do
      should allow_value("molly@mollyblack.com", "a@b.co.uk", "molly+hash@example.com").for(:email)
    end
    describe "should be invalid if email is not formatted correctly" do
      it { should_not allow_value("mollytaryn.com").for(:email) }
      it { should_not allow_value("molly@examplecom").for(:email) }
